...
void StartScene::m_updateUI()
{
	...
	if (ImGui::SliderFloat("PPM (Pixels Per Meter)", &m_PPM, 1.0f, 30.0f, "%.1f"))
	{

	}

	if (ImGui::SliderFloat("Kicking Angle (Angle)", &m_angle, 0.0f, 90.0f, "%.1f"))
	{
...
void StartScene::m_move()
{
	...
	//m_finalPosition = m_pShip->getPosition() + (m_velocity * m_time) + ((m_acceleration * 0.5f) * (m_Atime * m_Atime));
